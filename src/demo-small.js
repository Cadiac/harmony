...
run = () => {
  H = 0; // Halt the demo if esc is hit, takes one press to exit fullscreen first
  ...
  setTimeout(() => {
    ...
    L = (dt, k) => {
      ...
      // I simplified the math by just precalculating some of the constants here
      // using mass of 8 and 2 for the spheres, lengths 3 and 4 and G = 10.
      A1 = 4 / 15;
      A2 = 3 / 4;
      ...
      // Apply some damping at the end of the demo to make the pendulum stop
      g1 =
    ...
    function render() {
      ...
      t = now;
      if (t > 45e3 || H) {
        // If demo is halted with esc or 45 seconds have elapsed exit
        // Disconnect the volume in case we're halting the demo
      ...
      // Calculate the position of the spheres by angle
      x = S(p[0]) * 3;
      y = -C(p[0]) * 3;

      // Mess with the lowpass filter and Q using pendulum's velocity and angle
