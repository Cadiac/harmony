...
run = () => {
  ...
  // Pink noise
  const bufferSize = a.sampleRate * 4; // 4 seconds of audio
  ...
  return document.documentElement.requestFullscreen().then(async () => {
    e = performance.now();
    t = 0;
    frame = 0;
    ...
    // gl.shaderSource(
    //   shader,
    //   `precision highp float;uniform float v;uniform vec2 d;uniform vec4 m;const vec3 i=vec3(.8,.7,.6),M=vec3(1,.92,1),e=vec3(.7,.8,.9);struct Material{vec3 d;vec3 m;vec3 e;float h;float f;};struct Surface{int i;float d;Material m;};struct Ray{Surface d;vec3 n;vec3 o;bool h;};float n(float v){return fract(v*17.*fract(v*.3183099));}float f(vec3 v){vec3 d=floor(v),M=fract(v),c=M*M*(3.-2.*M);float m=d.x+317.*d.y+157.*d.z,e=n(m),i=n(m+1.),o=n(m+317.),z=n(m+318.),f=n(m+157.),h=n(m+158.),y=n(m+474.),S=n(m+475.);return-1.+2.*(e+(i-e)*c.x+(o-e)*c.y+(f-e)*c.z+(e-i-o+z)*c.x*c.y+(e-o-f+y)*c.y*c.z+(e-i-f+h)*c.z*c.x+(-e+i+o-z+f-h-y+S)*c.x*c.y*c.z);}float t(vec3 v){float m=0.,M=.5;for(int i=0;i<7;i++){float c=f(v);m+=M*c;M*=.5;v=2.*mat3(0.,.8,.6,-.8,.36,-.48,-.6,-.48,.64)*v;}return m;}mat3 s(float v){float m=sin(v),M=cos(v);return mat3(vec3(1,0,0),vec3(0,M,-m),vec3(0,m,M));}mat3 l(float v){float m=sin(v),M=cos(v);return mat3(vec3(M,0,m),vec3(0,1,0),vec3(-m,0,M));}mat3 r(float v){float m=sin(v),M=cos(v);return mat3(vec3(M,-m,0),vec3(m,M,0),vec3(0,0,1));}float f(vec3 v,vec3 m,vec3 e){vec3 i=v-m,M=e-m;return length(i-M*clamp(dot(i,M)/dot(M,M),0.,1.))-.05;}float f(vec3 v,float m){return length(v)-m;}float h(vec3 v){vec3 m=abs(v)-vec3(2);return length(max(m,0.))+min(max(m.x,max(m.y,m.z)),0.);}float p(vec3 v){vec3 M=vec3(m.xy,0),e=vec3(m.zw,0);float i=f(v,.3),d=f(v-M,.8),h=f(v-e,.5),c=f(v,vec3(0),M),z=f(v,M,e);return min(min(i,min(d,h)),min(c,z));}Surface h(Surface m,Surface v){if(m.d<v.d){return m;} return v;}Surface c(vec3 m){Surface i,M=Surface(0,p(m-vec3(0,9,0)),Material(vec3(.5),vec3(.5),vec3(.5),50.,.5)),d=Surface(1,dot(m,vec3(0,1.5,0))+2.,Material(vec3(1),vec3(.5),vec3(.5),2.,0.)),e=Surface(2,f(m-vec3(5),5.),Material(vec3(.5),vec3(.5),vec3(.5),50.,.5)),z=Surface(3,f(m-vec3(-2,2,-3),3.),Material(vec3(1,.3,.2),vec3(.9,.5,.5),vec3(.9),50.,.3)),c=Surface(4,h(s(v*1e-4)*l(v*7e-4)*(m-vec3(-8,5,3))),Material(vec3(.5,.5,.8),vec3(.1),vec3(.9),50.,.5));i=h(d,M);i=h(i,e);i=h(i,z);return h(i,c);}vec3 c(vec3 m,vec3 v,vec3 d){vec3 f=e-.5*v.y,c=exp2(-abs((2.5e4-m.y)/v.y)*1e-5*M);f=f*c+(1.-c)*i;float h=dot(d,v);if(h>.9999)f=vec3(.9);return f;}float c(vec3 v,vec3 m){float i=1.,M=1.;for(int f=0;f<400;++f){if(M>=250.)return i;Surface d=c(m+M*v);if(d.d<1e-5)return 0.;i=min(i,10.*d.d/M);M+=d.d;}return i;}mat4 S(vec3 v){vec3 m=normalize(vec3(0,5,0)-v),M=normalize(cross(normalize(vec3(0,1,0)),m));return mat4(vec4(M,0),vec4(cross(m,M),0),vec4(-m,0),vec4(0,0,0,1));}vec3 S(vec3 v,vec3 m,vec3 d,vec3 e,float f,Material h){float l=c(v,d);vec3 z=exp2(-f*.05*M);return(h.m+h.d*clamp(dot(v,m)*l,0.,1.)+h.e*pow(clamp(dot(reflect(v,m),e),0.,1.),h.h))*z+(1.-z)*i;}Ray S(vec3 m,vec3 v){float i=1e-5,M=1e-5;Ray f;for(int d=0;d<400;d++){i=.001*M;f.o=m+M*v;f.d=c(f.o);if(f.d.d<i){f.h=true;break;}M+=f.d.d*.5;if(M>=250.)break;}f.d.d=M;return f;}vec3 l(vec3 m,vec3 f){vec3 M=normalize(vec3(0,10,v/5e2)),i=vec3(0);float d=1.;vec3 e=f;float z=0.;Ray h=S(m,e);for(int o=0;o<4;o++){if(!h.h){i=mix(i,c(m,e,M),d);break;}vec3 y=vec3(0);if(h.d.i==1)y=vec3(0,1,0);else if(h.d.i==2)y=normalize(h.o-.5*t(r(v*1e-4)*h.o*5.)-vec3(5));else if(h.d.i==3)y=normalize(h.o-vec3(-1,2,-3));else{const vec2 l=vec2(1,-1);y=normalize(l.xyy*c(h.o+l.xyy*1e-5).d+l.yyx*c(h.o+l.yyx*1e-5).d+l.yxy*c(h.o+l.yxy*1e-5).d+l.xxx*c(h.o+l.xxx*1e-5).d);}z+=h.d.d;vec3 l=S(M,y,h.o,e,z,h.d.m);i=mix(i,l,d);d*=h.d.m.f;if(d<1e-5)break;e=reflect(e,y);h=S(h.o,e);}if(!h.h||h.d.i==1){float l=clamp(dot(M,e),0.,1.);i+=.5*vec3(1,.5,.2)*pow(l,32.);}return i;}void main(){vec2 m=gl_FragCoord.xy-d/2.;vec3 i=vec3(-20.+20.*sin(v/5e3),4.+sin(v/2500.),-20.+20.*cos(v/5e3));mat4 f=S(i);vec3 h=l(i,(f*vec4(normalize(vec3(m,-d.y/tan(radians(60.)/2.))),0)).xyz);h=pow(h,M);h*=vec3(1.02,.99,.9);h.z=h.z+.1;h=smoothstep(0.,1.,h);gl_FragColor=vec4(h,1);}`
    // );
    ...
    function render() {
      ...
      gl.uniform4f(gl.getUniformLocation(program, "u_p"), x, y, xx, yy);
